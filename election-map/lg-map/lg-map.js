$(function() {
  ...
  $.getScript( mapWrapper.data('map'), function() {
    ...
    $.ajax({
      type: "GET",
      url: "http://thepolitic.org/wp-content/themes/the_politic_2016/election-map/lg-map/states-project.csv",
      dataType: "text",
      success: function(data) {
        stateData = $.csv.toObjects(data);
      },
    ...
    function createMap() {
      ...
      var boxattrs = {};
      var i = 0;
      var colors = ['#f09fa5', '#decda7', '#9a89bf', '#c999c1', '#8dd4f4', '#94b1db', '#d5e29d', '#99cd9f', '#f1ad4f', '#e5d34d', '#f19a7c', '#f4b589', '#68c4af']
      for (var state in paths) {
        ...
        if (!paths[i].enable) {
          ...
          paths[i].color = colors[i % colors.length]
          switch(i){
            case 5:
            paths[i].color = colors[colors.length - 1];
            break;
            case 33:
            paths[i].color = colors[colors.length - 1];
            break;
            case 20:
            paths[i].color = colors[3];
            break;
            case 44:
            paths[i].color = colors[9];
            break;
            case 28:
            paths[i].color = colors[6];
            break;
            case 23:
            paths[i].color = colors[4];
            break;
            case 35:
            paths[i].color = colors[2];
            break;
            case 32:
            paths[i].color = colors[0];
            break;
            case 19:
            paths[i].color = colors[5];
            break;
          }
          boxattrs = {
            'fill': paths[i].color,
      ...
      $('#myModal').on('show.bs.modal', function (event) {
        ...
        for (var i = 0; i < students.length; i++) {
          if (stateData[curid][students[i]] != '') {
            var name = $("<h4>");
            name.text(stateData[curid][students[i]])
            var classyear = students[i] + ' Class Year'
            var studentq1 = students[i] + ' Q1'
            var studentq2 = students[i] + ' Q2'
            if (stateData[curid][classyear] != '') {
              name.text(name.text() + ', \'' + stateData[curid][classyear].substr(2, 2))
            }
            body.append(name);
            var question1ans = $("<p>", {style: "font-family: 'Swift';"});
            question1ans.text(stateData[curid][studentq1])
            body.append(question1ans);
            var question2 = $("<h5>", {style: 'font-weight: bold;'});
            question2.text("Prediction for home state:");
            body.append(question2);
            var question2ans = $("<p>", {style: "font-family: 'Swift';"});
            question2ans.text(stateData[curid][studentq2])
            body.append(question2ans)
          }
        }
      });
